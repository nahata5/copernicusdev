...
class RegistrationPage:UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        case 9:
            cell = tableView.dequeueReusableCellWithIdentifier("spanish") as UITableViewCell
        default:
            println()
        }
        if isgoingToEditProfile {
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        view.addConstraint(NSLayoutConstraint(item: tableView, attribute: .Height, relatedBy: .Equal, toItem: nil, attribute: .NotAnAttribute, multiplier: 0, constant: 44*10))
        table = tableView
        return 10
    }
}